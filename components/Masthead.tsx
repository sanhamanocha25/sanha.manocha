...
export function Masthead() {
  ...
  // Track which section is in view so the nav can mark it like a coded line.
  useEffect(() => {
    if (pathname !== "/") return;
    const ids = nav
      .map((n) => n.href)
          ...
          <ul className="mono flex items-center gap-x-5 text-[0.78rem] leading-none">
            {nav.map((item) => {
              ...
              const current =
                ...
                (!isNotes && pathname === "/" && item.href === `/#${active}`);
              return (
                <li key={item.href} className="shrink-0">
                  <Link
                    ...
                    className={`focus-marker inline-block py-2 no-underline transition-colors duration-200 ${
                      current ? "bg-marker px-1.5 text-ink" : "text-pencil hover:text-ink"
                    }`}
                  >
                    {item.label}
                  </Link>
